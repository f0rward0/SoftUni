...
namespace PCCatalog
{
    class Computer
    {
        ...
        public void PrintComputerInformation()
        {
            ...
            if (computerProcessor.Price > 0)
            {
                Console.WriteLine("--> {0} price: {1}BGN", computerProcessor.Name, computerProcessor.Price);
            }
            if (computerGraphicsCard.Price > 0)
            {
                Console.WriteLine("--> {0} price: {1}BGN", computerGraphicsCard.Name, computerGraphicsCard.Price);
            }
            if (computerMotherboard.Price > 0)
            {
                Console.WriteLine("--> {0} price: {1}BGN", computerMotherboard.Name, computerMotherboard.Price);
            }
            if (this.CalculateComputerPrice() > 0)
            {
                Console.WriteLine("Total Price: {0}BGN", this.CalculateComputerPrice());
            }
        }
    }
}